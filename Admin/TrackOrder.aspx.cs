...
join (select OrderMain_OrderMainID,min(Item_ItemID)Item_ItemID,min(Quantity)Quantity,min(Price)Price from 
tbl_OrderSub group by OrderMain_OrderMainID) os on os.OrderMain_OrderMainID=om.OrderMainID
join tbl_ItemMaster as im on im.ItemID=os.Item_ItemID
join tbl_ItemImages as ii on ii.Item_ItemID=im.ItemID
join tbl_User as u on om.User_UserID=u.UserID where OrderStatus='" + OrderStatus + "'";
...
om.OrderNumber,om.ShipDate,om.OrderStatus,om.IsDelivered,os.Item_ItemID,os.Quantity,os.Price,im.ItemName,u.UserName FROM tbl_OrderMain as om 
join (select OrderMain_OrderMainID,min(Item_ItemID)Item_ItemID,min(Quantity)Quantity,min(Price)Price from 
tbl_OrderSub group by OrderMain_OrderMainID) os on os.OrderMain_OrderMainID=om.OrderMainID
join tbl_ItemMaster as im on im.ItemID=os.Item_ItemID join tbl_ItemImages as ii on ii.Item_ItemID=im.ItemID
join tbl_User as u on om.User_UserID=u.UserID where IsDelivered!='Y' and OrderStatus='Confirmed' order by OrderDate desc";
            DataSet ds = DB_Con.GetDS(query);
            dt = ds.Tables[0];
            if (dt.Rows.Count > 0)
            {
        ...
        //{
        //    if (ddlStatus.SelectedItem.Text == "Shipped")
        //    {
            ...
            try
            {
                ...
                if (ddlStatus.SelectedIndex > 0)
                {
                    ...
                    if (ChkStatus == "True")
                    {
                        ...
                        //{
                        //    DateTime ED = Convert.ToDateTime(txtfromdate.Text.Trim());
                        //    lTrackOrder_BAL.lTrackOrder_CDAL.ExpectedDeliveryDate = ED.ToString("yyyy/MM/dd");
                        //}
        ...
        private void SendEmail(string OrderMainID)
        {
            ...
            for (int j = 0; j < listTrackOrder_DAL.Count; j++)
            {
                string email = "";
                DataTable dt = new DataTable();
                //                string query = @" select  os.ordersubid,os.ordermain_ordermainid,om.ShippingAddress1,im.ItemName,im.ItemCode,ii.ImageName,om.OrderNumber,om.User_UserID,
                //u.UserName,u.EmailID,u.ContactNo,os.item_itemid,os.quantity,os.price,om.BillAmt,om.totalamount,om.ShippingAmt from tbl_ordersub as os join tbl_ordermain as om 
                //on os.ordermain_ordermainid=om.OrderMainID";
                //                query = query + " join tbl_ItemMaster as im on im.ItemID=os.item_itemid";
                //                query = query + " join tbl_ItemImages as ii on ii.Item_ItemID=im.itemid";
                //                query = query + " join tbl_User as u on u.UserID=" + listTrackOrder_DAL[j].User_UserID + "";
                //                query = query + " where om.OrderMainID=(select max(OrderMainID) from tbl_ordermain where User_UserID=" + listTrackOrder_DAL[j].User_UserID + ")";
                //                query = query + "order by ordersubid desc ";
...
where om.OrderMainID=" + listTrackOrder_DAL[j].OrderMainID + " order by ordersubid desc";
                ...
                MailAddress ma = new MailAddress("info@myfettlebioproducts", "Myfettle Bio Products - Nutritional Supplements & Health Mix for Babies");
                if (email != null)
                {
                    string stremailTo = email;
                    mail.Subject = "Your Myfettle Bio Products Order has been delivered - successfully  OrderNo(" + dt.Rows[0]["OrderNumber"] + ")";
   ...
   <table bgcolor='#fafafa' width='100%' height='100%' align='center' style='border-collapse:collapse' border='0'
      ...
      <tbody>
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td width='48' valign='top'
                           ...
                           <img src='http://myfettlebioproducts.com/image/main-assert/logo.png' border='0' class='CToWUd'></td>
                        <td align='right'
                           style='font-size:12px;line-height:18px;color:#666;border-top:none;border-bottom:none;border-left:none;border-right:none'>
                            <p style='letter-spacing: 0.5px;font-size: 14px;color: #000;'>Nutritional Supplements &
                              <br>
                              <span style='font-weight:bold;'>Health Mix For Babies</span>
                           </p>
                        </td>
                        <td width='20' style='border-top:none;border-bottom:none;border-left:none;border-right:none'>
                        </td>
               ...
               <table width='600' border='0' cellspacing='0' cellpadding='0' bgcolor='#ffffff' style='text-align:left;border: white;'>
                  <tbody>
                     ...
                     <tr>
                        ...
                        mail.Body += @"<br>
                              <span style='font-size:24px;font-weight: bold;letter-spacing: 0.5px;'>Your MyFettle
                                Order is now delivered</span>
                              <br>
                              Thank you for shopping with us on myfettlebioproducts.com!
                  ...
                  <tbody>
                     <tr>
                        ...
                        <td width='20%' align='top' style='border:none;'>
                          <img src='http://myfettlebioproducts.com/ItemImage/" + dt.Rows[i]["ImageName"].ToString() + "' width='140' height='200' />";
                            mail.Body += @"</td>
                        <td width='60%' bgcolor='#fff' style='border:none;'>
                           <table border='1' bordercolor='#fff' cellspacing='0' cellpadding='0' bgcolor='#ffffff'
                              ...
                              <tbody>
                                 <tr>
                                    <td height='8' width='100%' valign='top' bgcolor='#baeca4'
                           ...
                           <table border='1' bordercolor='#fff' cellspacing='0' cellpadding='0' bgcolor='#ffffff'
                              ...
                              <tbody>
                                 <tr>
                                    ...
                                    <td width='70%' valign='top' bgcolor='#baeca4'
                                       style='color: #000;padding: 10px;font-size: 12px;font-weight: bold;border: 1px solid #fff;'>
                                      " + dt.Rows[i]["ItemCode"] + "";
                            mail.Body += @"</td>
                                 </tr>
                           ...
                           <table border='1' bordercolor='#fff' cellspacing='0' cellpadding='0' bgcolor='#ffffff'
                              ...
                              <tbody>
                                 <tr>
                                    ...
                                    <td width='60%' valign='top' bgcolor='#baeca4'
                                       style='color: #000;padding: 10px;font-size: 12px;font-weight: bold;border: 1px solid #fff;'>
                                       Rs." + dt.Rows[i]["price"] + "";
                            mail.Body += @"</td>
                                    <td width='10%' valign='top' bgcolor='#baeca4'
                                       style='color: #000;padding: 10px;font-size: 12px;font-weight: bold;border: 1px solid #fff;'>
                                     " + dt.Rows[i]["quantity"] + "";
                            mail.Body += @"</td>
                                 </tr>
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td align='top' style='border-top:none;border-bottom:none;border-left:none;border-right:none'>
                          <p><a href='http://myfettlebioproducts.com/index.aspx'
                                 style='color: #fff;text-decoration: none;'>www.myfettlebioproducts.com</a></p>
                        </td>
                        <td align='right'
                           style='font-size:12px;line-height:18px;color:#666;border-top:none;border-bottom:none;border-left:none;border-right:none'>
                           <p style='letter-spacing: 0.5px;font-size: 14px;color: #000;'>
                               <ul class='social'>
                                 <li><a href='#'><img src='http://myfettlebioproducts.com/image/fb.png' width='20' height='20' alt='' /></a></li>
                                 <li><a href='#'><img src='http://myfettlebioproducts.com/image/twit.png' width='20' height='20' alt='' /></a></li>
                                 <li><a href='#'><img src='http://myfettlebioproducts.com/image/insta.png' width='20' height='20' alt='' /></a></li>
                              </ul>
                           </p>
                           <br>
                        </td>
               ...
               <table width='600' border='0' bordercolor='#ffffff' cellspacing='0' cellpadding='0' bgcolor='#f2f2f2'
                  ...
                  <tbody>
                     <tr>
                        ...
                        <td align='top' style='border-top:none;border-bottom:none;border-left:none;border-right:none'>
                           <p
                              ...
                              <br>
                              Copyrights &#169; 2021 <span style='font-size: 13px;font-weight:bold;'>www.myfettlebioproducts.com</span>
                              | All rights reserved
                           </p>
                        </td>
                        <td width='20' style='border-top:none;border-bottom:none;border-left:none;border-right:none'>
                     ...
                     <tr>
                        ...
                        //smtp.EnableSsl = true;
                        NetworkCredential NetworkCred = new NetworkCredential("hello@myfettlebioproducts.com", "fettlehello19#");
        ...
        protected void btnconfirm_Click(object sender, EventArgs e)
        {
            ...
            TrackOrder_BAL lTrackOrder_BAL = new TrackOrder_BAL();
            lTrackOrder_BAL.lTrackOrder_CDAL = new TrackOrder_CDAL();
            lTrackOrder_BAL.lTrackOrder_CDAL.OrderStatus = "Confirmed";
        ...
        protected void btnship_Click(object sender, EventArgs e)
        {
            ...
            TrackOrder_BAL lTrackOrder_BAL = new TrackOrder_BAL();
            lTrackOrder_BAL.lTrackOrder_CDAL = new TrackOrder_CDAL();
            lTrackOrder_BAL.lTrackOrder_CDAL.OrderStatus = "Shipped";
            listTrackOrder_CDAL = lTrackOrder_BAL.GetStatus();
            Gridordersub.DataSource = listTrackOrder_CDAL;
            Gridordersub.DataBind();
        }
        protected void btndispatch_Click(object sender, EventArgs e)
        {
            ...
            TrackOrder_BAL lTrackOrder_BAL = new TrackOrder_BAL();
            lTrackOrder_BAL.lTrackOrder_CDAL = new TrackOrder_CDAL();
            lTrackOrder_BAL.lTrackOrder_CDAL.OrderStatus = "Dispatched";
            listTrackOrder_CDAL = lTrackOrder_BAL.GetStatus();
            Gridordersub.DataSource = listTrackOrder_CDAL;
            Gridordersub.DataBind();
        }
        protected void btndeliver_Click(object sender, EventArgs e)
        {
            ...
            TrackOrder_BAL lTrackOrder_BAL = new TrackOrder_BAL();
            lTrackOrder_BAL.lTrackOrder_CDAL = new TrackOrder_CDAL();
            lTrackOrder_BAL.lTrackOrder_CDAL.OrderStatus = "Delivered";
            listTrackOrder_CDAL = lTrackOrder_BAL.GetStatus();
            Gridordersub.DataSource = listTrackOrder_CDAL;
            Gridordersub.DataBind();
        }
        protected void View(object sender, CommandEventArgs e)
        {
            ...
            Response.Redirect("ViewOrder.aspx?orderno=" + orderno);
        }

        //        private void Orderdetail(int orderno)
        //        {
        //            string query = "";
        //            query = @"SELECT om.OrderMainID,om.User_UserID,ii.ImageName,om.OrderDate,om.TotalAmount,om.OrderNumber,om.ShipDate,om.OrderStatus,om.IsDelivered,
        //os.Item_ItemID,os.Quantity,os.Price,im.ItemName,u.UserName
        //FROM tbl_OrderMain as om join tbl_OrderSub as os on os.OrderMain_OrderMainID=om.OrderMainID
        //join tbl_ItemMaster as im on im.ItemID=os.Item_ItemID
        //join tbl_ItemImages as ii on ii.Item_ItemID=im.ItemID
        //join tbl_User as u on om.User_UserID=u.UserID where IsDelivered!='Y' and OrderMainID='" +orderno +"'";
        //            DataSet ds = DB_Con.GetDS(query);
        //            dt = ds.Tables[0];
        //            if (dt.Rows.Count > 0)
        //            {
        //                ViewState["UserId"] = dt.Rows[0]["User_UserID"];
        //                Gridordersub1.DataSource = dt;
        //                Gridordersub1.DataBind();
